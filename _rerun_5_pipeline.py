"""
KSE-30 INDEX FUND FLOW & REBALANCING ANALYSIS â€” FULL PIPELINE
==============================================================
Single file covering the complete research workflow:

  SECTION 0  â€” Configuration & shared utilities
  SECTION 1  â€” Data ingestion & cleaning  (kse30_daily_data.csv)
  SECTION 2  â€” Master dataset construction (daily + monthly)
  SECTION 3  â€” Exploratory data analysis
  SECTION 4  â€” GARCH volatility modelling
  SECTION 5  â€” Fund flow prediction  (ARIMAX + VAR)
  SECTION 6  â€” Market efficiency tests
  SECTION 7  â€” KSE-30 rebalancing weight & inclusion prediction
  SECTION 8  â€” Results summary

Input files (same directory as this script)
  kse30_daily_data.csv   â€” KSE-30 constituent daily data (2020-01-01 â†’ present)
  funds_data.xlsx        â€” AKD / NBP / NTI NAV & AUM sheets
  macro_data.xlsx        â€” OIL / IR / USD sheets
  cpi.csv                â€” Monthly CPI YoY
...
# SECTION 0 â€” CONFIGURATION & UTILITIES
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

BASE     = r'D:\roohan\fyp-indexfunds\5_claude_pipeline'
RERUN_OUT = r'D:\roohan\fyp-indexfunds\_rerun_outputs\5_claude_pipeline'
FIG_BASE = os.path.join(RERUN_OUT, "figures")
...
# â”€â”€ normalised company name map (SYMBOL â†’ clean name) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
COMPANY_MAP = {
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# SECTION 1 â€” DATA INGESTION & CLEANING
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

print("\n" + "="*65)
print("SECTION 1 â€” Data ingestion & cleaning")
print("="*65)
...
# Handle volume column â€” check if both "Volume" and "VOLUME" exist
...
# Convert share/mcap columns â€” dash ("-") entries are trading-halt placeholder rows
...
print("Computing per-symbol features â€¦")
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# SECTION 2 â€” MASTER DATASET CONSTRUCTION
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

print("\n" + "="*65)
print("SECTION 2 â€” Master dataset construction")
print("="*65)
...
print(f"Daily master: {len(daily):,} rows Ã— {len(daily.columns)} cols | "
      f"{daily.date.min().date()} â†’ {daily.date.max().date()}")
...
print(f"Monthly master: {len(monthly):,} rows Ã— {len(monthly.columns)} cols | "
      f"{monthly.date.min().date()} â†’ {monthly.date.max().date()}")

# Save master datasets
daily.drop(columns=["month"]).to_csv(
    os.path.join(RERUN_OUT, "rerun_daily_master.csv"), index=False
)
monthly.drop(columns=["month"], errors="ignore").to_csv(
    os.path.join(RERUN_OUT, "rerun_monthly_master.csv"), index=False
)
...
print("\n" + "="*65)
print("SECTION 3 â€” Exploratory data analysis")
print("="*65)

# â”€â”€ 3.1 Descriptive stats: daily NAV returns â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
print("\n[Table] Descriptive statistics â€” Daily NAV log returns")
desc_rows = []
...
# â”€â”€ 3.2 AUM trend â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
ax.set_title("Monthly AUM â€” AKD, NBP, NIT (PKR Millions)")
...
# â”€â”€ 3.3 NAV return distributions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
for i, (fund, col) in enumerate([("AKD","nav_return_akd"),
    ...
    ax.set_title(f"{fund}  Î¼={s.mean():.4f}  Ïƒ={s.std():.4f}\n"
                 f"skew={s.skew():.2f}  kurt={s.kurtosis():.0f}")
    ax.set_xlim(s.quantile(0.01), s.quantile(0.99))
plt.tight_layout()
savefig("eda", "rerun_E02_nav_return_dist.png")

# â”€â”€ 3.4 Fund flows bar chart â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
print("\n" + "="*65)
print("SECTION 4 â€” GARCH volatility modelling")
print("="*65)
...
for fund, col in [("AKD","nav_return_akd"),
    ...
    print(f"\n  {fund} â€” fitting GARCH(1,1) and EGARCH(1,1) â€¦")
...
pd.DataFrame(garch_rows).to_csv(
    os.path.join(RERUN_OUT, "rerun_results_garch.csv"), index=False
)
...
for row, (fund, col) in enumerate([("AKD","nav_return_akd"),
    ...
    axes[row][0].set_title(f"{fund} â€” Daily NAV Returns (%)")
    axes[row][0].set_ylabel("Return (%)")
    axes[row][1].plot(dt, vol, color=FUND_COLORS[fund], linewidth=1)
    axes[row][1].set_title(f"{fund} â€” GARCH(1,1) Cond. Vol (%)")
...
print("\n" + "="*65)
print("SECTION 5 â€” Fund flow prediction")
print("="*65)
...
print("\n  Granger causality (macro â†’ fund flow):")
gc_rows = []
...
for macro_col, label in [("interest_rate_end","IR â†’ flow"),
                          ("cpi_yoy_end","CPI â†’ flow"),
                          ("oil_return_monthly","Oil â†’ flow"),
                          ("usdpkr_return_monthly","USD/PKR â†’ flow")]:
...
# â”€â”€ 5.5 Individual fund ARIMAX â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
fund_ff_results = {}
...
# â”€â”€ 5.6 Fund flow figures â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
ax.set_title("Total Fund Flow â€” Actual vs ARIMAX vs VAR(1) (PKR Millions)")
...
print("\n" + "="*65)
print("SECTION 6 â€” Market efficiency tests")
print("="*65)
...
# Efficiency figures
fig, axes = plt.subplots(1,3,figsize=(14,4))
fig.suptitle("ACF â€” Daily NAV Returns", fontweight="bold")
for i,(fund,col) in enumerate([("AKD","nav_return_akd"),
                                 ("NBP","nav_return_nbp"),
                                 ("NIT","nav_return_nti")]):
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# SECTION 7 â€” REBALANCING WEIGHT & INCLUSION PREDICTION
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

print("\n" + "="*65)
print("SECTION 7 â€” Rebalancing weight & inclusion prediction")
print("="*65)
...
print("\n  Feature comparison â€” retained vs dropped:")
...
for _, row in future_df.iterrows():
    flag = " âš " if row["exclusion_risk"]>0.35 else ""
...
ax.set_title(f"Predicted Retention Probability â€” Next KSE-30 Rebalancing\n"
...
fig.suptitle("Feature Importances â€” Rebalancing Models", fontweight="bold")
...
ax.set_title(f"Weight Prediction â€” Test Set\nRidge RÂ²={m_ridge_wt['R2']:.4f}")
plt.tight_layout()
...
print("\n" + "="*65)
print("SECTION 8 â€” Results summary")
print("="*65)

print("\n[Table 1] Descriptive statistics â€” Daily NAV returns")
print(desc_df.to_string(index=False))
...
print("\n[Table 7] At-risk stocks â€” next rebalancing")
...
fig.suptitle("KSE-30 Fund Flow & Rebalancing Analysis â€” Results Dashboard",
