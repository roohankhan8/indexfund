...
# Winsorize company returns at 1%/99% quantiles to fix data outliers like BOP
q_low = company_monthly['return'].quantile(0.01)
q_high = company_monthly['return'].quantile(0.99)
company_monthly['return'] = company_monthly['return'].clip(lower=q_low, upper=q_high)
print(f"    Company returns winsorized [{q_low:.1%}, {q_high:.1%}]")
...
upper_line = ax.axhline(upper_w, color='darkorange', linewidth=1.2, linestyle='--')
lower_line = ax.axhline(lower_w, color='darkorange', linewidth=1.2, linestyle='--')
ax.legend([upper_line, lower_line], [f'+3σ = {upper_w:.0f}', f'−3σ = {lower_w:.0f}'], fontsize=9)
