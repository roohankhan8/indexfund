"""
Transform 1b data into stationarity-friendly modeling series and report results.

Outputs (report_workspace/chapter-03-methodology/images):
...
SRC_1B = ROOT / "1b"
OUT = ROOT / "report_workspace" / "chapter-03-methodology" / "images"
...
def load_data():
    raw = pd.read_csv(SRC_1B / "kse30_daily_data.csv").rename(
        columns={
            "Date": "date",
            "PRICE": "price",
            "IDX WT %": "weight_pct",
    ...
    clean = pd.read_csv(SRC_1B / "kse30_stocks_clean.csv")
    ...
    for fund in ["AKD", "NBP", "NTI"]:
        df = pd.read_excel(SRC_1B / "funds_data.xlsx", sheet_name=fund).rename(
