...
Input files read from local folder ./data/raw/
...
Outputs (under ./output/analysis/): daily_master.csv, monthly_master.csv, results_*.csv, figures/

Run (from repo root):

   python production_pipeline/pipeline.py
...
OUT_DIR = os.path.join(_SCRIPT_DIR, "output", "analysis")
...
# ── 3.1 Descriptive stats: KSE-30 index (reconstructed) daily log returns ─────
...
# ── 3.2 Combined sector AUM (index-tracking funds) ────────────────────────────
...
ax.set_title("KSE-30 daily log return - distribution\n"
             f"μ={s.mean():.4f}  σ={s.std():.4f}  skew={s.skew():.2f}")
...
row0 = eff_rows[0]
ax.set_title(f"{SER_EFF}  LB p={row0['LB Q p']:.4f}  ACF₁={row0['ACF lag-1']:.4f}")
ax.set_xlabel("Lag")
plt.tight_layout()
